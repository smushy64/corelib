...
attr_internal
u64 internal_hash_rotate( u64 val, int shift ) {
    return shift == 0 ? val : ((val >> shift) | (val << (64 - shift)));
}
attr_internal
u64 internal_hash_shift_mix( u64 val ) {
    return val ^ (val >> 47);
}
attr_internal
u64 internal_hash_unaligned_load_64( const char* p ) {
    u64 result;
    memory_copy( &result, p, sizeof(result) );
    return result;
}
attr_internal
u32 internal_hash_unaligned_load_32( const char* p ) {
    u32 result;
    memory_copy( &result, p, sizeof(result) );
    return result;
}
...
u64 internal_hash_bswap_64( u64 v ) {
    u8* bytes = (u8*)&v;
...
attr_unused attr_internal
u32 internal_hash_bswap_32( u32 v ) {
    u8* bytes = (u8*)&v;
    u8 tmp;
...
attr_internal
u32 internal_fetch_32( const char* p ) {
...
attr_internal
struct InternalHashPair internal_hash_weak_hash_len_32_with_seeds_0(
...
attr_internal
struct InternalHashPair internal_hash_weak_hash_len_32_with_seeds_1(
...
attr_internal
u64 internal_hash_128_to_64( struct InternalHashUInt128 v ) {
...
attr_internal
u64 internal_hash_len_16_0( u64 u, u64 v ) {
    return internal_hash_128_to_64(
        (struct InternalHashUInt128){ .upper=u, .lower=v } );
}
attr_internal
u64 internal_hash_len_16_1( u64 u, u64 v, u64 mul ) {
    u64 a = ( u ^ v ) * mul;
    a ^= ( a >> 47 );
    u64 b = ( v ^ a ) * mul;
    b ^= ( b >> 47 );
...
attr_internal
hash64 internal_hash_len_0_to_16( usize len, const char* s ) {
    if( len >= 8 ) {
        u64 mul = K2 + len * 2;
...
attr_internal
hash64 internal_hash_len_17_to_32( usize len, const char* s ) {
    u64 mul = K2 + len * 2;
...
attr_internal
hash64 internal_hash_len_33_to_64( usize len, const char* s ) {
    u64 mul = K2 + len * 2;
    u64 a   = internal_hash_fetch_64( s ) * K2;
    u64 b   = internal_hash_fetch_64( s + 8 );
...
attr_core_api
hash64 hash_city_64( usize len, const void* _buf ) {
    const char* buf = _buf;
    if( len <= 32 ) {
        if( len <= 16 ) {
