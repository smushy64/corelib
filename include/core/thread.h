...
attr_core_api
b32 thread_join_timed( ThreadHandle* handle, u32 ms, int* opt_out_exit_code );
/// @brief Wait for thread to finish execution for infinite time.
/// @param[in]  handle            Handle of thread to join with.
/// @param[out] opt_out_exit_code (optional) Pointer to write thread exit code to.
attr_always_inline attr_header
void thread_join( ThreadHandle* handle, int* opt_out_exit_code ) {
...
attr_core_api
b32 thread_exit_code( ThreadHandle* handle, int* out_exit_code );
